...
// Changes the slider value to a percent width, no longer nested in determineDx
...
var determineDx = function(elem, size, windowwidth) {
  // Expose window width input so it's calculated only once
...
var changePizzaSizes = function(size) {
  // Select pizza elements only once, not for each iteration of loop!
  var pizzaElements = document.querySelectorAll(".randomPizzaContainer");
  // Calculate windowwidth, dx, newwidth only once
...
var resizePizzas = function(size) {
  ...
  // Clearing this nested mess, no need to recreate function objects for changeSliderLabel,
  // and changePizzaSizes
...
function updatePositions() {
  ...
  var pixelsScrolledScaled = document.body.scrollTop / 1250 // Calculate only once
  var items = document.querySelectorAll('.mover');
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(pixelsScrolledScaled + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  for (var i = 0; i < 40; i++) { // Reducing pizza number to speed up rendering
